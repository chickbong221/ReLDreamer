"""Local replacement for ``mshab.envs.wrappers.collect_data.FetchCollectRobotInitWrapper``.

The upstream wrapper has a bug in ``__init__``: it iterates
``self._base_env.bc_to_task_plans`` directly. That attribute is
``Dict[str, List[TaskPlan]]`` (see ``mshab/envs/sequential_task.py:166``), so
iterating yields ``build_config_name`` *strings*. The wrapper then treats each
element as a list of subtasks and runs ``len(tp) == 1`` -- i.e. it asks
"is the build-config-name a single character?", which is never true. The
"Must have only one subtask" assertion therefore fires on every call.
